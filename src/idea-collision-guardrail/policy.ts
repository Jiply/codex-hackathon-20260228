...
export class DefaultCollisionPolicy implements CollisionPolicy {
  ...
  evaluate(matches: SimilarityMatch[]): CollisionDecision {
    ...
    const semanticScores = ranked.map((match) =>
      Number(match.metadata?.semantic_score ?? match.score ?? 0)
    );
    const lexicalScores = ranked.map((match) =>
      Number(match.metadata?.lexical_score ?? 0)
    );
    ...
    const highNeighborCount = semanticScores.filter(
      (score) => score >= this.config.rejectMultiScore
    ).length;
